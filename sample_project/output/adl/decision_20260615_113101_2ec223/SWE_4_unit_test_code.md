...
- Decision: decision_20260615_113101_2ec223
...
static void test_invalid_sensor_requests_safe_state(void)
{
    ThermalMgr_Input input = { .coolant_temp_c = 25, .sensor_valid = false };
    ThermalMgr_Output output;

    ThermalMgr_Evaluate(&input, &output);

    assert(output.fan_enable == true);
    assert(output.pump_enable == true);
    assert(output.thermal_fault == true);
    assert(output.derate_request == true);
}

static void test_temperature_85_requests_cooling_and_derate(void)
{
    ThermalMgr_Input input = { .coolant_temp_c = 85, .sensor_valid = true };
    ThermalMgr_Output output;
    ...
    assert(output.fan_enable == true);
    assert(output.pump_enable == true);
    assert(output.thermal_fault == false);
    assert(output.derate_request == true);
}

int main(void)
{
    test_invalid_sensor_requests_safe_state();
    test_temperature_85_requests_cooling_and_derate();
    return 0;
}
```