...
def _semantic_intent(question: str) -> str:
    ...
    if any(token in text for token in ["知识", "模板", "经验", "规范", "学习", "knowledge", "template", "ÖªÊ¶", "Ä£°å", "¾­Ñé"]):
        return "knowledge"
    if any(token in text for token in ["基线", "评审", "批准", "review", "baseline", "»ùÏß", "ÆÀÉó"]):
...
def _fake_answer_for_intent(intent: str, prompt: str) -> str:
    if intent == "conversation_quality":
        return (
            ...
            "当前修复后的测试模式会把普通工程问题转交给证据型回答器，直接引用 AgentTask、产物、Gate、追溯、评审和知识库记录；"
    ...
    if intent == "model_identity":
        ...
        return (
            ...
            + "平台的任务规划和对话会经过 PersonalLLMGateway；PSL 的 Goal Model、资源检查、失败归因，ADL 的候选动作、受控执行、Gate、追溯和评审落库，以及证据型问答，都是规则、数据库和工具链共同组成的闭环。"
        )
    if intent == "psl_explanation":
        return (
            "PSL 是 Problem-Solving Loop，也就是问题解决闭环。它负责维护持续目标、拆解可执行步骤、读取项目资源、调用受控动作、"
            "记录失败归因和证据结果；ADL 则负责把这些动作限制在 ASPICE 的产物、追溯、Gate、评审和基线边界内。"
        )
    if intent == "autonomous_capability":
        return (
            "可以，但这里的“自主完成”不是无边界自动乱改。平台会先用 PSL 维护目标和步骤，再通过 ADL 选择受控动作；"
            "高风险结果只生成候选产物并进入 Gate 与人工评审，不能直接绕过评审写入正式基线。"
        )
