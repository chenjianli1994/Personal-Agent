...
PERMISSIONS: dict[str, str] = {
    ...
    "artifact.write": "创建和更新工程产物",
    "gate.run": "执行质量 Gate 检查",
    "review.request": "发起评审",
    "review.decide": "批准或驳回评审",
    "baseline.approve": "批准基线",
    "knowledge.read": "查看受控知识库文档",
    "knowledge.write": "维护知识库文档",
    "agent.run": "运行受控单 Agent 任务",
    "code_integration.approve": "批准代码合入",
    "audit.read": "查看审计与证据记录",
}

ROLE_DEFINITIONS: dict[str, dict[str, Any]] = {
    "system_admin": {"scope": "organization", "name": "系统管理员", "description": "平台最高权限，可管理组织、项目、成员、权限、基线、知识库、Agent 任务、代码合入与审计。", "permissions": list(PERMISSIONS)},
    ...
    "project_admin": {"scope": "project", "name": "项目管理员", "description": "项目内最高权限，可维护项目成员、角色、责任矩阵、评审、基线、Agent 任务和代码合入。", "permissions": list(PERMISSIONS)},
    "process_owner": {"scope": "project", "name": "过程负责人", "description": "负责项目过程执行和产物闭环，可维护产物、运行 Gate、发起评审并查看知识与审计记录。", "permissions": ["project.read", "artifact.write", "gate.run", "review.request", "knowledge.read", "audit.read"]},
    "requirement_owner": {"scope": "project", "name": "需求负责人", "description": "负责需求维护与需求评审发起，可创建和更新需求并查看审计记录。", "permissions": ["project.read", "requirement.write", "review.request", "audit.read"]},
    "developer": {"scope": "project", "name": "开发人员", "description": "负责工程产物实现，可维护产物、运行受控单 Agent 任务并查看审计记录。", "permissions": ["project.read", "artifact.write", "agent.run", "audit.read"]},
    "tester": {"scope": "project", "name": "测试人员", "description": "负责测试与质量检查，可维护测试相关产物、运行 Gate 并查看审计记录。", "permissions": ["project.read", "gate.run", "artifact.write", "audit.read"]},
    "reviewer": {"scope": "project", "name": "评审人员", "description": "负责评审结论，可查看项目并批准或驳回评审。", "permissions": ["project.read", "review.decide", "audit.read"]},
    "approver": {"scope": "project", "name": "批准人员", "description": "负责正式批准，可批准评审、基线和代码合入。", "permissions": ["project.read", "review.decide", "baseline.approve", "code_integration.approve", "audit.read"]},
